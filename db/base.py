...
class BaseBaseModel(object):
    ...
    def timestamp(seconds=None):
        if seconds:
            return long(time.mktime(time.gmtime(seconds))*1000)
        else:
            return long(time.mktime(time.gmtime())*1000)
        #return long(time.time()*1000)

    @convert_to_record
    def to_one_str(self, value):
        """string化的单个record对象
        """
